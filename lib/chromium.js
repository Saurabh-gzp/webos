  ...
  async runTask(actions, opts = {}) {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (step.keep !== true) {
        ...
        if (step.include === 'text') entry.text = (session.text || '').slice(0, Number(step.limit) || 4000);
        else if (step.include === 'elements' || step.includeElements) entry.elements = (session.elements || []).slice(0, Number(step.limit) || 40);
      }
      results.push(entry);
