...
function seed() {
  ...
  r.children.Home.children['AGENT.md'] = fileNode(
    ...
    [
      ...
      '',
      '',
      'REAL CHROMIUM (asli browser, iframe-proxy nahi) — headless Chromium server pe chalta hai:',
      '  POST /api/browser                    {url,width,height}  -> {sessionId}',
      '  POST /api/browser/task               {actions:[{action:"navigate",url:...},{action:"extract"},{action:"screenshot"}]}',
      '  POST /api/browser/<id>/navigate      {url}',
      '  POST /api/browser/<id>/act           {action:"click|type|press|scroll|eval|extract|screenshot|state", ...}',
      '  GET  /api/browser/<id>/screenshot    asli image (vision ke liye)',
      '  GET  /api/browser/<id>/state         url, title, elements[] (stable refs)',
      '  GET  /api/browser/<id>/extract       text + headings + links',
      '  GET  /api/browser/<id>/stream        SSE live view (screencast)',
      '  bus ops: chromium.open | chromium.goto | chromium.act | chromium.task | chromium.screenshot | chromium.frame | chromium.status | chromium.close',
      '',
